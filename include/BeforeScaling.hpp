...
void BeforeScalingOnlyAC(TH1D* SE, TH1D* Background, TPaveText* lSys, TString outname){
  // --- Create TObjArrays -----------------------------------------------------
  ...
  main->Add(lSys);
  std::unique_ptr<Legend> l(new Legend(main.get(), "angle cut rejected\n same event", "lp lp"));
  l->SetFillStyle(0);

  // --- Marker ----------------------------------------------------------------

  vector<Color_t> colors = {kMagenta-3, kBlack, 0, 0};
  vector<Style_t> markers = {kOpenSquare, kFullCircle, 0, 0};
  vector<Size_t>  sizes = {2., 2., 0, 0};

  // --- Canvasses -------------------------------------------------------------

  Legend::SetPosition(l.get(), 0.6, 0.9, 0.860, 0.920);

  SquarePlot square = SquarePlot(main.get(), minv_str, count_str);
  square.SetMode(Plot::Thesis);
  square.SetStyle(colors, markers, sizes);
  square.SetRanges(0.0, 1.6, SE->GetMinimum(), Background->GetMaximum());
  square.SetCanvasMargins(0.025, .1, 0.03, .1);
