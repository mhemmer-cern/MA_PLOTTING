...
SquarePlot FitAfterScalig(TH1D* SE, TH1D* Background1, TH1D* Background2, TPaveText* lSys){
  // --- Create TObjArrays -----------------------------------------------------
  ...
  main->Add(lSys);
  TLegend* l = Legend(main, "same event\n background pol1\n background pol2", "lp lp lp").GetLegendPointer();

  // --- Marker ----------------------------------------------------------------

  vector<Color_t> colors = {kBlack, kCyan-4, kPink-3, 1, 1};
